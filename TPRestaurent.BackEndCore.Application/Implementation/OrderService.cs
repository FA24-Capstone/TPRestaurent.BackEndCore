    ...
    public class OrderService : GenericBackendService, IOrderService
    {
            ...
            try
            {
                ...
                await _unitOfWork.ExecuteInTransaction(async () =>
                {
                    ...
                    else if (orderRequestDto.OrderType == OrderType.Delivery)
                    {
                        ...
                        {
                            throw new Exception($"Không tìm thấy địa chỉ {accountDb.Address} hoặc tài khoản chưa lựa chọn địa chỉ chính để giao hàng");
                        }
        ...
        public async Task<AppActionResult> UploadConfirmedOrderImage(ConfirmedOrderRequest confirmedOrderRequest)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (accountDb == null)
                    {
                        throw new Exception($"Không tìm thấy khách hàng với id {orderDb.AccountId}");
                    }

                    if (confirmedOrderRequest.RefundRequired)
                    {
                        var newTransaction = new Transaction
                        {
                            ...
                            PaymentMethodId = PaymentMethod.STORE_CREDIT,
                            TransactionTypeId = TransactionType.Refund,
                            Date = orderDb.CancelledTime.Value,
                            Amount = hashingService.Hashing(accountDb.Id, orderDb.TotalAmount, false).Result.ToString(),
                            OrderId = orderDb.OrderId,
                            TransationStatusId = TransationStatus.SUCCESSFUL
                        };
                        int storeCreditAmount = 0;
            ...
            try
            {
                ...
                if (cancelDeliveringOrderRequest.isCancelledByAdmin == true)
                {
                    ...
                    var newTransaction = new Transaction
                    {
                        ...
                        TransactionTypeId = TransactionType.Refund,
                        Date = currentTime,
                        Amount = hashingService.Hashing(accountDb.Id, order.TotalAmount, false).Result.ToString(),
                        OrderId = order.OrderId,
                        TransationStatusId = TransationStatus.SUCCESSFUL
                    };
        ...
        public async Task<AppActionResult> CancelOrderDetailBeforeCooking(List<Guid> orderDetailIds)
        {
            ...
            try
            {
                ...
                var groupedDishCraftService = Resolve<IGroupedDishCraftService>();
                var updateDishSizeDetailList = new List<DishSizeDetail>();
                var orderDetailDb = await _detailRepository.GetAllDataByExpression(d => d.Order.OrderTypeId != OrderType.Delivery 
                ...
                if (orderDetailDb.Items.Count > 0)
                {
                    ...
                    {
                        var otherOrderDetailDb = await _detailRepository.GetAllDataByExpression(o => o.OrderSessionId == orderSession.OrderSessionId && !orderDetailIds.Contains(o.OrderDetailId), 0, 0, null, false, null);
                        if (otherOrderDetailDb.Items.All(o => o.OrderDetailStatusId == OrderDetailStatus.Cancelled))
                        {
                            orderSession.OrderSessionStatusId = OrderSessionStatus.Cancelled;
                        }
                        else if (otherOrderDetailDb.Items.All(o => o.OrderDetailStatusId == OrderDetailStatus.Cancelled || o.OrderDetailStatusId == OrderDetailStatus.ReadyToServe))
                        {
